...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const { data: perfil, error } = await ProfileService.getProfile(user.id);
...
function renderDashboard(user, perfil) {
    const nombre = perfil?.nombre || user.user_metadata?.full_name || user.user_metadata?.name || "Miembro";
    const apellido = perfil?.apellido || "";
    const nombreCompleto = `${nombre} ${apellido}`.trim();
    const correo = user.email || "";
    const rol = user.user_metadata?.role || "Miembro";
    ...
    setText("welcomeTitle", `¡Bienvenido, ${primerNombre}!`);
...
function renderAvatar(photo, nombre) {
    ...
    if (photo) {
        if (topAvatarImage) { topAvatarImage.src = photo; topAvatarImage.style.display = "block"; }
        if (sidebarAvatarImage) { sidebarAvatarImage.src = photo; sidebarAvatarImage.style.display = "block"; }
        if (topAvatarInitials) topAvatarInitials.style.display = "none";
        if (sidebarAvatarInitials) sidebarAvatarInitials.style.display = "none";
        return;
    }

    const initials = getInitials(nombre);
    if (topAvatarInitials) { topAvatarInitials.textContent = initials; topAvatarInitials.style.display = "block"; }
    if (sidebarAvatarInitials) { sidebarAvatarInitials.textContent = initials; sidebarAvatarInitials.style.display = "block"; }
}
...
function getInitials(name) {
    return name.trim().split(" ").filter(Boolean).slice(0, 2).map(part => part[0]).join("").toUpperCase();
}
