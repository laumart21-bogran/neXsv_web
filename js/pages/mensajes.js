...
function formatConversationTime(value) {
    ...
    if (sameDay) {
        return new Intl.DateTimeFormat("es-SV", { hour: "2-digit", minute: "2-digit" }).format(date);
    }
    return new Intl.DateTimeFormat("es-SV", { day: "2-digit", month: "short" }).format(date);
}
...
async function getProfileName(userId) {
    if (!userId) return "Miembro neXsv";
...
async function enrichConversations(rows) {
    return Promise.all((rows || []).map(async row => {
        ...
        const name = await getProfileName(otherUserId);
        ...
        return {
            ...
            lastMessage,
            hasUnread,
            sortDate: new Date(lastMessage?.created_at || conversation.updated_at || conversation.created_at || 0).getTime()
        };
    }));
}
...
function renderConversationList(items = conversations) {
    ...
    conversationList.innerHTML = items.map(item => {
        ...
        return `
            <button class="conversation-item ${isActive ? "active" : ""} ${item.hasUnread ? "unread" : ""}" type="button" data-conversation-id="${escapeHtml(item.conversation.id)}">
                <div class="messages-avatar">${escapeHtml(initials(item.name))}</div>
...
async function openConversation(conversationId) {
    ...
    activeOtherUserId = item.otherUserId;
    ...
    activeChannel = await messagingService.subscribeToMessages(conversationId, async message => {
        if (message.sender_id !== currentUser.id) {
            const existing = [...messageList.querySelectorAll(".message-bubble")].some(node => node.textContent.includes(message.body));
            if (!existing) {
                await loadMessages(conversationId);
            }
            await messagingService.markConversationRead(conversationId);
...
conversationSearch.addEventListener("input", () => {
    ...
    renderConversationList(conversations.filter(item => {
        const preview = item.lastMessage?.body || "";
        return item.name.toLowerCase().includes(term) || preview.toLowerCase().includes(term);
    }));
});
...
async function initialize() {
    try {
        ...
        topAvatar.textContent = initials(currentUser.email || "Miembro");
        await loadConversations();
