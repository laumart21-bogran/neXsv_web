...
function initials(name="Miembro") {
    return String(name).trim().split(/\s+/).filter(Boolean).slice(0,2).map(p=>p[0]?.toUpperCase()).join("") || "M";
}
...
function showMessage(text, type="") {
    ...
    publicationMessage.className = `form-message ${type}`.trim();
}
...
async function enrichPublications(publications) {
    return Promise.all((publications || []).map(async p => ({...p,author:await getAuthorProfile(p.author_id)})));
}
...
publicationForm?.addEventListener("submit",async event=>{
    ...
    document.querySelectorAll(".type-option").forEach(o=>o.classList.toggle("active",o.querySelector("input")?.checked));
