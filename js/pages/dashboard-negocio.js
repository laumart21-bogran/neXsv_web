...
function renderSummary(businesses) {
    ...
    setText("businessActive", businesses.filter(b => normalize(b.estado) === "activo").length);
    setText("businessPending", businesses.filter(b => normalize(b.estado) !== "activo").length);
}
    ...
    return `<article class="business-publication-card" data-publication-id="${escapeAttr(publication.id)}">
        ...
        <div class="business-publication-content">
            ...
            <div class="business-publication-metrics">
                <div class="business-publication-metric"><strong data-publication-metric="views">0</strong><span>Vistas</span></div>
                <div class="business-publication-metric"><strong data-publication-metric="comments">0</strong><span>Comentarios</span></div>
                <div class="business-publication-metric"><strong data-publication-metric="conversations">0</strong><span>Conversaciones</span></div>
            </div>
...
async function loadPublicationMetrics(publicationIds) {
    ...
    setText("businessViews", totals.views); setText("businessComments", totals.comments); setText("businessConversations", totals.conversations);
...
function resetPublicationTotals() { setText("businessViews", 0); setText("businessComments", 0); setText("businessConversations", 0); setText("businessMessages", 0); }
...
function renderBusinesses(businesses) {
    const container = document.getElementById("businessList"); if (!container) return;
    if (!businesses.length) { container.innerHTML = `<div class="business-empty-state"><i class="fa-solid fa-store"></i><strong>Aún no tienes un negocio registrado</strong><span>Incorpora tu primer negocio para comenzar.</span><a href="incorporar-negocio.html" class="business-primary-btn">Incorporar negocio</a></div>`; return; }
...
function renderError() { setText("businessTotal", "—"); setText("businessActive", "—"); setText("businessPending", "—"); const container = document.getElementById("businessList"); if (container) container.innerHTML = `<div class="business-empty-state"><i class="fa-solid fa-triangle-exclamation"></i><strong>No pudimos cargar tus negocios</strong><span>Intenta nuevamente en unos momentos.</span></div>`; }
